                ...
                {sidebarItem.map((group, groupIndex) => (
                    <div key={groupIndex}>
                        ...
                        {group.items.map((item, itemIndex) => (
                            <Accordion key={itemIndex} type="single" collapsible className="px-3 sidebar-accordion">
                                        ...
                                        <ul>
                                            {item.links.map((subItem, subIndex) => (
                                                <li className="pl-6" key={subIndex}>
                                                    <Link
                                                        to={subItem.to}
                                                        className={`side-menu__item block text-[#a3aed1] text-13px relative hover:text-white p-2 rounded-lg hover:bg-[rgba(255,255,255,0.5)] ${
                                                            location.pathname.includes(subItem.to) ? 'bg-white text-black' : ''
                                                        }`}
                                                    >
                                                        {subItem.title}
                                                    </Link>
